...
6) Created file called 'cc_gas.py' with the method 'cc_burn' in it that takes market, model, volume, demaState, tbp, tsp, and cc from the market group brokers and generates 4 unique variables from that set of data
- cc_burn must return a string of 4 variables seperated by commas like: str(var1) + "," + str(var2) + "," + str(var3) + "," + str(var4);
- These 4 variables are then turned into a feature set to be used for training and predicting
